...
@router.get("/{sensor_id}/data", response_model=Point)
async def get_sensor_data(sensor_id: str) -> Point:
    ...
    return Point(time=time.time(), value=corrected)


@router.get("/{sensor_id}/data/history", response_model=PointsList)
...
@router.get("/{sensor_id}/raw", response_model=Point)
async def get_sensor_raw_data(sensor_id: str) -> Point:
    ...
    return Point(time=time.time(), value=raw_value)


@router.put("/{sensor_id}/zero", status_code=204)
