...
class TestSensorConnectionErrorHandling:
    ...
    def test_get_sensor_data_returns_503_when_disconnected(self, client, hardware_mode):
        ...
        if "FORCE" in sensor_reconnection_manager.monitors:
    ...
    def test_get_graphique_returns_503_when_force_disconnected(self, client, hardware_mode):
        """GET /graph/{sensor_name} should return 503 if FORCE not connected."""
        # Set FORCE as disconnected (hardware_mode ensures emulation doesn't override)
        if "FORCE" in sensor_reconnection_manager.monitors:
            sensor_reconnection_manager.monitors["FORCE"].state = SensorState.DISCONNECTED
    ...
    def test_get_graphique_base64_returns_503_when_disp1_disconnected(self, client, hardware_mode):
        ...
        if "DISP_1" in sensor_reconnection_manager.monitors:
            sensor_reconnection_manager.monitors["DISP_1"].state = SensorState.DISCONNECTED
    ...
    def test_get_graphique_arc_returns_503_when_disp2_disconnected(self, client, hardware_mode):
        ...
        if "DISP_2" in sensor_reconnection_manager.monitors:
